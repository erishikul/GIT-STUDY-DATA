...
@section('content')
    ...
    <div class="container">
        ...
        <div class="tb-main-header" style=" box-shadow: rgb(0 0 0 / 15%) 1.95px 1.95px 2.6px;">
            ...
            {{-- <p class="test-title pull-left hidden-sm mar-t16 mar-l16 mar-b0 test-mwebtitle ng-binding">Puzzle of the Day - Box Puzzle</p> --}}

            <img src="{{ asset('assets/images/logo.png') }}" alt="" class="test-title pull-left hidden-sm mar-t16 mar-l16 mar-b0 test-mwebtitle ng-binding"  style="width:130px; height:30px;">
        ...
        {{-- @if (isset($_GET['key']) && $_GET['key'] >= $total_ques)
            <div class="col-md-7 mb-5" style="margin: auto">
                <div class="map-content-9 text-center">
                    ...
                    <a href="?explaintion" class="btn btn-warning">View Explaintion</a>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-8  mb-5"  style="height:80vh;">
                       <div style="padding:5px 0 25px 0" >
                        ...
                        <div class="">
                            <h6> Que {{ $key + 1 }} : <span class="{{ $test->ques[$key]->language }}">
                        ...
                        <form action="?" method="get">
                            ...
                            <div class="row col-md-12">
                                <div class="form-check col-md-6">
                                    <input class="form-check-input mt-2 mr-2 " type="submit" name="option"
                                        value="{{ $test->ques[$key]->options[0]->is_correct }}" id="flexRadioDefault1" style="curser:pointer; border-radius: 50px; color: white;" >
                                    <label class="form-check-label {{ $test->ques[$key]->language }} @if($test->ques[$key]->options[0]->is_correct == 1) text-success font-weight-bold @endif"
                                ...
                                <div class="form-check col-md-6">
                                    <input class="form-check-input mt-2 mr-2" type="submit" name="option"
                                        value="{{ $test->ques[$key]->options[1]->is_correct }}" id="flexRadioDefault2" style="curser:pointer; border-radius: 50px; color: white;">
                                    <label class="form-check-label {{ $test->ques[$key]->language }} @if($test->ques[$key]->options[1]->is_correct == 1) text-success font-weight-bold @endif"
                            ...
                            <div class="row col-md-12">
                                <div class="form-check col-md-6">
                                    <input class="form-check-input mt-2 mr-2" type="submit" name="option"
                                        value="{{ $test->ques[$key]->options[2]->is_correct }}" id="flexRadioDefault3" style="curser:pointer; border-radius: 50px; color: white;">
                                    <label class="form-check-label {{ $test->ques[$key]->language }} @if($test->ques[$key]->options[2]->is_correct == 1) text-success font-weight-bold @endif"
                                ...
                                <div class="form-check col-md-6">
                                    <input class="form-check-input mt-2 mr-2" type="submit" name="option"
                                        value="{{ $test->ques[$key]->options[3]->is_correct }}" id="flexRadioDefault4" style="curser:pointer; border-radius: 50px; color: white;" >
                                    <label class="form-check-label {{ $test->ques[$key]->language }} @if($test->ques[$key]->options[3]->is_correct == 1) text-success font-weight-bold @endif"
                            ...
                            <hr>
                            <p> Explaination : <span class="text-success"> {{$test->ques[$key]->explanation}} </span></p>
                            <div class="">
                    ...
                    <div class="col-md-4">
                        <div class="col-12 map-content-9" style=" background-color: #15acab54;     padding: 20px;">
                            <div style="width:100%">
                                <img @if (session('user')->profile_picture != null)
                                src="{{ asset('images/users/'.session('user')->profile_picture) }}"
                                @else src="{{ asset('images/users/user.jpeg') }}" @endif alt=""
                                    style="width: 29px;
                            height: 29px;
                            border-radius: 50px;">
                            ...
                            {{-- <span> <span style="background:red; width:25px; height:25px;">0</span> Answord </span> <span> marked</span> <span>not visite</span> <span>not answord</span> --}}
                            {{-- <div class="text-center">
                                <span style="background-color: #00800061;border-radius: 10px;padding: 0 9px;" > Time-Left::5:10</span>
                            </div> --}}
                        ...
                        <div class="map-content-9 text-center">
                            ...
                            <p>Total Right : {{ $right }}</p>
                            <p class="text-success font-weight-bold">Your Score : {{ ($right/$total_ques) * 100 }} %</p>
                            <a href="{{route('test_start', base64_encode($test->id))}}" class="btn btn-info">Start Again</a>
                            <a href="{{route('profile')}}" class="btn btn-danger">Go Back to Profile</a>
                        </div>
                            ...
                            @endfor
                            <hr>
                            <a href="?key={{ $total_ques }}&skip=" class="btn btn-info">View Score Board</a> --}}
                        </div>
                    </div>
                @else
