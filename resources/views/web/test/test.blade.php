...
<body>
    ...
    <section class="header">
        <nav>
            ...
            <span class="navbtn">
                {{-- <button class="navSFC" onclick="requestFullScreen()">Switch Full Screen</button>
    ...
    <section class="container1">
        <div class="lside">
            ...
            <hr>
            <form action="#" id="QueSubmit" style="height: 73%">
                <div class="test_question">
                    <div class="quest_ions" id="ques">
                        Here is Your Question
                    </div>
                    <div class="options">
                        <p ><input type="radio" name="options" id="1option1" value="1"> <label
                                id="option1" for="1option1">1</label></p>
                        <p><input type="radio" name="options" id="2option2" value="2"> <label
                                id="option2" for="2option2">2</label></p>
                        <p><input type="radio" name="options" id="3option3" value="3"> <label
                                id="option3" for="3option3">3</label></p>
                        <p><input type="radio" name="options" id="4option4" value="4"> <label
                                id="option4" for="4option4">4</label></p>
                    </div>
                </div>
                <input type="hidden" name="que_id" id="que_id">
                <input type="hidden" name="itr" id="itr" value="">
                <hr>
                <style>
                    .btn {
                        background-color: #6695d248;
                ...
                <div class="save_next">
                    <button type="submit" onclick="mark_and_next()" >Mark for Review & Next</button>

                    <button type="reset">Clear</button>
                    <button type="submit" onclick="save_and_next()" style="float: right;">Save & Next</button>
                </div>
            </form>
        </div>
        <div class="rside">
            ...
            <div class="checklist">
                <span class="checkupper">
                    <span>
                        <i class="num1" id="attempted">0</i>
                        <i class="answered">Attempted</i>
                    </span>
                    <span>
                        <i class="num2" id="marked">0</i>
                        <i class="marked">Marked</i>
                    </span>
                    <span>
                        <i class="num3" id="not_visited">0</i>
                        <i class="not_visited">Not Visited</i>
                    </span>
                </span>
                <span class="checklower">
                    <span>
                        <i class="num4" id="mark_ans">0</i>
                        <i class="mark_ans">Marked and answered</i>
                    </span>
                    <span>
                        <i class="num5" id="NotAnswered">0</i>
            ...
            <div class="columns_bar">
                ...
                <span id="items">7</span>
                <span id="items">8</span>
                <span id="items">9</span>
                <span id="items">10</span> --}}
            </div>
            <hr>
            <div class="submit_bar">
                ...
                <br>
                <a href="{{ route('test_done', base64_encode($data->id)) }}" ><button class="submit_test">Submit
                        Test</button></a>
            </div>
        </div>
    </section>
    ...
    <script>
        ...
        function que_change(itr) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    window.console.log(result.selected);
                    if(result.selected){
                        op_id =  result.selected+'option'+result.selected
                        $('#'+op_id).attr('checked', true);
                    }else{
                        $('#1option1').attr('checked', false);
                        $('#2option2').attr('checked', false);
                        $('#3option3').attr('checked', false);
                        $('#4option4').attr('checked', false);
                    }
                }
            });
        ...
        $('#que_id').val(array[itr].id);
        var que_status = '';
        function mark_and_next(){
            window.que_status = 'mark';
        }
        function save_and_next(){
            window.que_status = 'save';
        }
        ...
        $("#QueSubmit").submit(function(event) {
            ...
            if (selected) {
                ...
                $('#queNo' + (itr)).addClass('mark');
            }

            if (selected != "") {
                if (array[itr - 1].options[selected - 1].is_correct == '1') {
                    right = true;
                } else {
                    right = false;
                }
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (result == 'done') {
                        // alert(result);
                        window.location.href = '{{ route('test_done', base64_encode($data->id)) }}';
                    } else {
                        $('#NotAnswered').text(result.NotAnswered);
                        $('#attempted').text(result.attempted);
                        $('#marked').text(result.marked);
                        $('#mark_ans').text(result.marked + result.attempted);
                        // alert(result.NotAnswered)
                        not_visited = total_ques - (result.NotAnswered + result.attempted + result.marked);
                        $('#not_visited').text(not_visited);
                    }
                }
            });
